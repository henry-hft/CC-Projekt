						...
						if ($_SERVER['REQUEST_METHOD'] === 'GET') { // get server(s)
							if (!empty($_GET['provider'])) { // only one provider
								...
								$response = array();
											...
											if(isset($providerName)){
												if($provider == $providerName){
													if($enabled == "1"){
														...
														if(!empty($_GET['id'])){
															$planResponse = $providerObj->plans($_GET['id']);
														} else {
															$planResponse = $providerObj->plans();
														}		
												...
												if($enabled == "1"){
													$providerObj = new $provider($providerToken);
													$planResponse = $providerObj->plans(null, true);
													
													if($planResponse['error'] == false){ 
														if(count($response) >= 1){
															$response = array_merge($response, $planResponse["plans"]);
														} else {
															$response = array_merge($response, $planResponse["plans"]);
														}
													} else {
									...
									if(!isset($providerName)){
										$response = array("error" => false, "plan" => $response);
									}
