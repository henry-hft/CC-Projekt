						...
						if ($_SERVER['REQUEST_METHOD'] === 'GET') { // get server(s)
							if (!empty($_GET['provider'])) { // only one provider
								...
								$response = array();
											...
											if(isset($providerName)){
												if($provider == $providerName){
													if($enabled == "1"){
														...
														if(!empty($_GET['id'])){
															$serversResponse = $providerObj->servers($_GET['id']);
														} else {
															$serversResponse = $providerObj->servers();
														}		
												...
												if($enabled == "1"){
													$providerObj = new $provider($providerToken);
													$serversResponse = $providerObj->servers(null, true);
													
													if($serversResponse['error'] == false){ 
														if(count($response) >= 1){
															$response = array_merge($response, $serversResponse["servers"]);
														} else {
															$response = array_merge($response, $serversResponse["servers"]);
														}
													} else {
									...
									if(!isset($providerName)){
										$response = array("error" => false, "servers" => $response);
									}
